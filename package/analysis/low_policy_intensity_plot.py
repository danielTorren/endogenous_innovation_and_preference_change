...
def plot_combined_policy_figures_with_utilty_cost_bottom(base_params, fileName, outputs, outputs_BAU, top_policies, dpi=300):
    fig, axs = plt.subplots(3, 2, figsize=(15, 12), sharex=True)
    time_steps = np.arange(base_params["duration_future"] - 1)
    start = base_params["duration_burn_in"] + base_params["duration_calibration"] 
    ...
    plt.tight_layout(rect=[0.01, 0.05, 0.98, 1])  # Leaves space at the bottom
    plt.subplots_adjust(wspace=0.15)  # increase spacing between columns
    plt.savefig(f"{fileName}/Plots/combined_policy_dashboard_with_utility_cost_bottom.png", dpi=dpi)
...
if __name__ == "__main__":
    main(fileName = "results/pair_low_intensity_policies_17_16_55__03_04_2025")#2D_low_intensity_policies_16_18_25__03_04_2025")
