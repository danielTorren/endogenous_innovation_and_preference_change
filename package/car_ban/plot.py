...
def _plot_metric(metric, results, expectation_modes, ban_years, ban_colors, save_dir):
    ...
    for col, mode in enumerate(expectation_modes):
        ...
        for year, color in zip(ban_years, ban_colors):
            r = results[(mode, f"ban_{year}")]
    ...
    path = f"{save_dir}/car_ban_{metric}.png"
...
def plot_time_series(results: dict, scenarios: list, save_dir: str = f"{RESULTS_DIR}/Plots"):
    ...
    ban_years = sorted({s["value"] for s in scenarios if s["kind"] == "ban"})

    ban_colors = plt.cm.viridis(np.linspace(0, 1, max(len(ban_years), 1)))

    figs = {}
    for metric in METRICS:
        figs[metric] = _plot_metric(metric, results, expectation_modes, ban_years, ban_colors, save_dir)
    return figs
