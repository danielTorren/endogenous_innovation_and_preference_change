...
def stochastic_produce_param_list_SA(
    ...
    for i, X in enumerate(param_values):
        ...
        for v in range(len(X)):  # loop through the properties to be changed
            base_params_copy[variable_parameters_dict_toList[v]["section"]][variable_parameters_dict_toList[v]["property"]] = X[
                v
...
def main(
    ...
    print("Total runs: ", len(params_list_sa_SW))
    
    res = parallel_run_multi_run(params_list_sa_SW)


    root = "multi_run_" + base_params["network_type"]
    fileName = produce_name_datetime(root)
    print("fileName:", fileName)

    createFolder(fileName)
    save_object(res, fileName + "/Data", "res")
