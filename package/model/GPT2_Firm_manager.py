...
from package.model.GPT2_firm import Firm
...
class Firm_Manager:
    ...
    def init_firms(self):
        ...
        self.global_neighbouring_technologies = {}
    ...
    def generate_neighbouring_technologies(self, firm):
        ...
        for string in firm.list_technology_memory_strings: 
            values = self.global_neighbouring_technologies[string]
            unique_neighbouring_technologies_strings.update(values)
    ...
    def update_memory(self, firm):
        ...
        #FOR NOW I GENERATE THE NEIGHBOURING TECHNOLOGIES FROM SCRATCH, GET THIS TO WORK THEN USE A COUNTER LATER
        firm.unique_neighbouring_technologies_strings  = self.generate_neighbouring_technologies(firm)

    def calculate_profitability_memory(self, firm, utilities_competitors):
        #print("firm.unique_neighbouring_technologies_strings", firm.unique_neighbouring_technologies_strings)
        #print("self.discovered_tech",self.discovered_tech)
        #quit()
    ...
    def last_tech_profitability(self, firm, utilities_competitors):
        last_tech_fitness_arr = np.asarray([firm.list_technology_memory[-1].attributes_fitness])
        ...
        if 0 not in denominators:
            ...
            expected_number_customer = self.segment_consumer_count * alternatives_probability_buy_car
            firm.last_tech_expected_profit = self.markup * last_tech_fitness_arr[0] * np.sum(expected_number_customer, axis=1)
        else:
            firm.last_tech_expected_profit = 0

    def rank_options(self, firm):
        firm.ranked_alternatives = []
        for tech, profitability in zip(firm.list_neighouring_technologies_strings, firm.expected_profit_research_alternatives):
            rank = 0
    ...
    def add_new_tech_memory(self, firm, chosen_technology):
        ...
        #THIS IS FOR ALL TECH FOR ALL FIRMS
        self.update_firm_manager_tech_list(self, chosen_technology)
    ...
    def gen_new_technology_memory(self,firm, selected_technology_string):
        ...
        researched_technology = Car(unique_tech_id, firm.firm_id, selected_technology_string, attribute_selected_tech, choosen_tech_bool=0, nk_landscape= self.nk_model)
    ...
    def research_technology(self, firm, utilities_competitors):
        ...
        self.calculate_profitability_memory(firm, utilities_competitors)
