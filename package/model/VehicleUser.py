...
"""
class VehicleUser:
    def __init__(self, user_id, chi, gamma, beta, nu, origin, vehicles_available, EV_bool, Kappa, alpha, d_i_min, delta, r, eta):
        ...
        self.nu = nu  # Time sensitivity
        self.origin = origin  # User's origin (urban or rural), impacts public transport availability
        self.EV_bool = EV_bool
        self.Kappa = Kappa  # Intensity of choice
        self.alpha = alpha  # Exponent in the optimal distance formula
        self.delta = delta  # Depreciation factor
        self.d_i_min = d_i_min  # Minimum required commuting distance
        self.r = r  # Discount rate
        self.eta = eta  # Scaling constant

        self.vehicle = self.decide_purchase(vehicles_available)  # Initial vehicle decision
        self.current_transportType = self.vehicle.transportType  # Current vehicle type
    ...
    def decide_purchase(self, vehicles_available):
        ...
        current_utility = self.calculate_utility(self.vehicle)

        # Calculate utility for each available vehicle
        utilities_new = []
        for vehicle in vehicles_available:
            utilities_new.append(self.calculate_utility(vehicle))  # Example scenario for new vehicles

        # Combine current vehicle utility with new vehicles
        utilities = np.array(utilities_new + [current_utility])
        ...
        all_vehicles = vehicles_available + [self.vehicle]
    ...
    def next_step(self, vehicles_available):
        ...
        """
        vehicle_chosen = self.decide_purchase(vehicles_available)
        return vehicle_chosen 